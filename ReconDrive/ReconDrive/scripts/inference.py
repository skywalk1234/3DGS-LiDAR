        ...
        if isinstance(output, tuple):
            ...
            if lidar_out is not None and output_dir:
                ...
                if lidar_gt is not None:
                    ...
                    # Compute LiDAR metrics (only on GT returns farther than 1.8m,
...
def save_lidar_ply(lidar_gt, lidar_out, scene_name, sample_idx, output_dir):
    ...
    Points closer than MIN_LIDAR_DEPTH (1.8m) are dropped: they are usually
    ego-vehicle self-scans that circle around the lidar.

    Returns the list of saved file paths.
    """
    MIN_LIDAR_DEPTH = 1.8  # meters; drop ego self-scan points
    ...
    # Pred points additionally masked by GT validity (same rays as GT, >1.8m)
