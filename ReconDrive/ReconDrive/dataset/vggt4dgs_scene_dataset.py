...
class NuScenesdataset4D(Dataset):
    ...
    def __getitem__(
        ...
        lidar_data = {}
        if source_sample is not None and 'LIDAR_TOP' in source_sample.get('data', {}):
            try:
                lidar_sample = self.dataset.get('sample_data', source_sample['data']['LIDAR_TOP'])
                ...
                # Use ego_to_lidar because Gaussians are in ego frame, not world frame
                # (depth2pc uses ego_to_camera extrinsic, output is in ego frame)
...
from dataset.data_util import build_lidar_range_image, LIDAR_NUM_RINGS, LIDAR_AZIMUTH_RESOLUTION
